...
int main(void)
{
 ...
 // HAL_TIM_Base_Start(&htim1); // Start TIM1
  ...
  // Read the actual PWR register to see current VOS mode


//uint32_t voltage_uv = 4563123;

//uint16_t voltage_dac = 0xFFFF;

//write_to_dac(3, 10, voltage_dac);
//uint16_t voltage_read = read_dac(3, 10);
...
//uint32_t elapsed = __HAL_TIM_GET_COUNTER(&htim1); // Read elapsed time
